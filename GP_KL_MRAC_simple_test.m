...
%is called GP-MRAC). In this case, the data in the training set will be
%discarded based on Kullback–Leibler (KL) divergence.
...
for ii = 1:length(T)-1
    ...
    [x_est, P_est] = KF_xdd(Y_meas(:,ii), x_est, P_est, A_k, H_k, R_k, Q_k);
    xdd_est_1 = x_est(3);               %Acceleration estimation
    xdd_est = xdd_t;
    accel_est_vec(1,ii) = xdd_est_1;
    
    delta = xdd_est - nu_pre;           %Actual modeling error
    delta_prop = xdd_est_1 - nu_pre;    %Modeling estimation error using acceleration estimate from Kalman Filter
    ...
    if gamma > eps_tol
        %If the training set is full, delete an element based on the KL
        %divergence method described in the paper
        if pnum == pmax
            %Delete a training point using the KL divergence method
            %described in the paper
            %Furthermore, after adding the new data set, compute new KZtau and Ctau, and obtain
            %values for the mean and covariance of the modelling error estimate
            %at the current state.
            %[delta_mean, delta_cov, KZtau, Ctau, XDATA, YDATA] = gpr_KL_update(Y_meas(:,ii)', delta, XDATA, YDATA, 10^log_noise_learn, kernel_sq);          %Use this for perfect (actual) modeling error feedback
            [delta_mean, delta_cov, KZtau, Ctau, XDATA, YDATA] = gpr_KL_update(Y_meas(:,ii)', delta_prop, XDATA, YDATA, 10^log_noise_learn, kernel_sq);     %Use this for estimated modeling error feedback
        else
            pnum = pnum + 1;
            ...
            %Adds new data set and updates matrices for future GPR
            %calculations
    ...
    %Obtaining true pseudo control input nu
    ...
    Y_meas(:,ii+1) = res + 0.1*randn(2, 1);     %Noisy state measurement
...
legend({'GP-MRAC Output (KL)','Reference Model'},'interpreter', 'latex', 'location', 'best')
hold off
...
legend(plots([2, 1, 3]), {'Estimated $\Delta (x)$ (KL)','True $\Delta x$', '$\Delta_{est} (x) \pm 2\sigma$'}, 'interpreter', 'latex', 'location', 'best')
hold off
...
function [x_est, P_est] = KF_xdd(z_meas, x_est_p, P_est_p, A, H, R, Q)
    ...
    %P_est = (eye(size(P_est_p)) - Kk*H)*P_pred;
...
%Gaussian Process Regression in which the covariance matrices are updated
%and the mean and covariance of the current estimated state (Gaussian Process) are obtained.
%Before that, however, an entry of the training set is deleted based on the
%KL divergence score that would result from the comparison between the
%full set and the training set without the previously mentioned data entry,
%such that the entry with the least score (which can be interpreted as the
%entry that offers the least information gain) is eliminated from the
%training set.
function [mean_predict, cov_predict, KZtau, Ctau, xtrainN, ytrainN] = gpr_KL_update(xpred, y_meas, xtrain, ytrain, noisecov, kernel_Handle)
    xtrain_mid = [xtrain; xpred];
    ytrain_mid = [ytrain; y_meas];
    Kztau_mid = build_covariance(xtrain_mid, xtrain_mid, kernel_Handle);
    vec_pred_mid = build_covariance(xpred, xtrain_mid, kernel_Handle);
    alpha_tau_p = (Kztau_mid\vec_pred_mid');
    Q_mid = inv(Kztau_mid);
    
    idx = deletedIdx(alpha_tau_p, Q_mid);
    
    xtrainN = [xtrain_mid(1:idx-1,:);xtrain_mid(idx+1:end,:)];
    ytrainN = [ytrain_mid(1:idx-1);ytrain_mid(idx+1:end)];
    
    KZtau = build_covariance(xtrainN, xtrainN, kernel_Handle);
    
    %Ctau = inv(KZtau + sqrt(noisecov)*eye(size(xtrainN,1)));
    Ctau = inv(KZtau + (noisecov^2)*eye(size(xtrainN,1)));
    
    mean_coeff = Ctau*ytrainN;
    vec_pred = build_covariance(xpred, xtrainN, kernel_Handle);
    mean_predict = vec_pred*mean_coeff;
    
    %cov_predict = zeros(size(xpred,1));
    cov_predict_pre = build_covariance(xpred,xpred,kernel_Handle);
    cov_predict_up = vec_pred*Ctau*vec_pred';
    cov_predict = cov_predict_pre-cov_predict_up;
end
...
%Determines the index corresponding to the data point in the GPR training
%set that whose elimination from the set would result in the least KL
%divergence "epsi" when comparing the complete set and the set without the
%corresponding data point
function idx = deletedIdx(alpha_k, Qn)
    qd = diag(Qn);
    epsi = abs(alpha_k)./qd;
    [~,idx] = min(epsi);
end
